module.exports = {
  id: 'lead-magnet-teaser-renovation',
  name: 'Renovation / Handyman Quote Teaser',
  category: 'lead-magnet',
  description: 'CTA section that launches the interactive renovation quote tool.',
  suitability: { renovation: 1.0, handyman: 0.9, construction: 0.8, contractor: 0.8 },
  contentSchema: {
  ...
  render(content, theme, sectionId = 'lmt-reno') {
    ...
    return `
<section class="${s}-wrap" style="background:#1a1200; padding:80px 20px; position:relative; overflow:hidden;">
  <style>
    .${s}-wrap { font-family: ${headFont}, sans-serif; }
    .${s}-bg { position:absolute; inset:0; background: url('https://images.unsplash.com/photo-1504307651254-35680f356dfd?w=1600&auto=format&fit=crop') center/cover no-repeat; opacity:0.2; }
    .${s}-inner { position:relative; z-index:1; max-width:720px; margin:0 auto; text-align:center; }
    .${s}-badge { display:inline-block; padding:6px 16px; border-radius:999px; background:${primary}22; border:1px solid ${primary}55; color:#fcd34d; font-size:13px; font-weight:600; letter-spacing:.5px; text-transform:uppercase; margin-bottom:24px; }
    .${s}-h { font-size:clamp(28px,4vw,44px); font-weight:800; color:#fff; line-height:1.2; margin-bottom:18px; }
    .${s}-sub { font-size:17px; color:#a89a70; line-height:1.6; max-width:560px; margin:0 auto 32px; }
    .${s}-pills { display:flex; flex-wrap:wrap; gap:10px; justify-content:center; margin-bottom:36px; }
    .${s}-pill { padding:7px 16px; border-radius:999px; background:#261d00; border:1px solid #3d2e00; color:#fcd34d; font-size:14px; font-weight:500; }
