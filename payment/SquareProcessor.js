...
class SquareProcessor extends PaymentProcessor {
  ...
  static getOAuthUrl(userId) {
    ...
    return `${baseUrl}/oauth2/authorize?client_id=${clientId}&scope=PAYMENTS_WRITE+PAYMENTS_READ+ORDERS_WRITE+ORDERS_READ+MERCHANT_PROFILE_READ+INVOICES_READ+INVOICES_WRITE+CUSTOMERS_READ+CUSTOMERS_WRITE&session=false&state=${userId}&redirect_uri=${encodeURIComponent(redirectUri)}`;
  }
