...
// ── Employee time off (owner-managed; shows on the app schedule) ─────────────
const TIME_OFF_DATE_RE = /^\d{4}-\d{2}-\d{2}$/;
...
router.post('/time-off', async (req, res) => {
  try {
    ...
    if (!TIME_OFF_DATE_RE.test(startDate) || !TIME_OFF_DATE_RE.test(endDate)) {
...
router.delete('/time-off/:id', async (req, res) => {
  try {
    const result = await pool.query(
      'DELETE FROM employee_time_off WHERE id = $1 AND user_id = $2 RETURNING id',
      [req.params.id, req.user.userId]
    );
    if (result.rows.length === 0) return res.status(404).json({ error: 'Not found' });
    res.json({ success: true });
  } catch (error) {
    console.error('Error deleting time off:', error.message);
