...
router.post('/login', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `SELECT ec.*, e.name as employee_name, e.user_id, e.color, e.active, e.is_admin, e.permissions,
...
router.get('/verify', authenticateEmployee, async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT e.id, e.name, e.color, e.active, e.is_admin, e.permissions, u.business_name, u.plan, u.email as owner_email,
              ec.email,
              bi.address as biz_address, bi.city as biz_city, bi.state as biz_state, bi.zip_code as biz_zip
       FROM employees e
       JOIN users u ON u.id = e.user_id
